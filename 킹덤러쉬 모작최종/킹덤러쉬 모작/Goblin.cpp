...
void CGoblin::Move()
{
	if (abs(int(m_vMoveRoute[m_currentRoute].x - GetPos().x)) > abs(int(m_vMoveRoute[m_currentRoute].y - GetPos().y)))
	{
		...
		{
			GetAnim()->PlayAnim(L"goblin_move_right", this, true);
		}
		else
		{
			GetAnim()->PlayAnim(L"goblin_move_left", this, true);
		}
	}
	else
	{
		//y축 애니메이션 실행
		if (m_vMoveRoute[m_currentRoute].y - GetPos().y > 0)
		{
			GetAnim()->PlayAnim(L"goblin_move_front", this, true);
		}
		else
		{
			GetAnim()->PlayAnim(L"goblin_move_back", this, true);
		}
	}
	Vec2 movePos = GetPos();
...
void CGoblin::OnCollision(CCollider * _other)
{
	// 솔저 혹은 영웅와 부뒷쳤을 때
		...
		|| _other->GetObj()->GetObjType() == OBJ_TYPE::ALLY_HERO)
	{
		// 내 타깃이 일치하는경우 공격한다.
		if(_other->GetObj()->GetObjType() == OBJ_TYPE::ALLY_UNIT
			&& ((CUnit*)_other->GetObj()) != GetTarget())
				return;
		else if (_other->GetObj()->GetObjType() == OBJ_TYPE::ALLY_HERO
			&& ((CHero*)_other->GetObj()) != GetTarget())
				return;
		...
		if (m_fAttTime > 3.f)
		{
			if (GetDirAtt() == UNIT_DIRECTION::RIGHT)
				GetAnim()->PlayAnim(L"goblin_attack_right", this, false);
			if (GetDirAtt() == UNIT_DIRECTION::LEFT)
				GetAnim()->PlayAnim(L"goblin_attack_left", this, false);
			if (_other->GetObj()->GetObjType() == OBJ_TYPE::ALLY_UNIT)
		...
		{
			if (GetDirAtt() == UNIT_DIRECTION::RIGHT)
				GetAnim()->PlayAnim(L"goblin_idle_right", this, true);
			if (GetDirAtt() == UNIT_DIRECTION::LEFT)
				GetAnim()->PlayAnim(L"goblin_idle_left", this, true);
		}
	}
}