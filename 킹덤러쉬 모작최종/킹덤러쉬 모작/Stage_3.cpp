...
void CStage_3::InitHud()
{
	...
	((CWave*)CWaveUI)->SetStageNum(3);
	CWaveUI->SetEnable(true);
	((CWave*)CWaveUI)->AddWave(1, MONSTER_TYPE::OGRE, 2 *2);
	((CWave*)CWaveUI)->AddWave(2, MONSTER_TYPE::GOBLIN, 1 *2);
	((CWave*)CWaveUI)->AddWave(3, MONSTER_TYPE::BANDIT, 3 *2);
	((CWave*)CWaveUI)->AddWave(3, MONSTER_TYPE::GOBLIN, 2 *2);
	((CWave*)CWaveUI)->AddWave(3, MONSTER_TYPE::OGRE, 1 *2);
	AddObject(CWaveUI, OBJ_TYPE::UI);
	m_myWave = (CWave*)CWaveUI;
	CStage_Field::InitHud();
}

void CStage_3::InitRoute()
{
...
void CStage_3::LoadingTex()
{
	CMapMgr::GetInst()->SetGold(500);
	CMapMgr::GetInst()->SetLife(10);
