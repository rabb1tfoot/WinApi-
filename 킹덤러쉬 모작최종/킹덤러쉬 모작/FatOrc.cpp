...
void CFatOrc::Move()
{
	if (abs(int(m_vMoveRoute[m_currentRoute].x - GetPos().x)) > abs(int(m_vMoveRoute[m_currentRoute].y - GetPos().y)))
	{
		...
		{
			GetAnim()->PlayAnim(L"fatorc_move_right", this, true);
		}
		else
		{
			GetAnim()->PlayAnim(L"fatorc_move_left", this, true);
		}
	}
	else
	{
		...
		{
			GetAnim()->PlayAnim(L"fatorc_move_front", this, true);
		}
		else
		{
			GetAnim()->PlayAnim(L"fatorc_move_back", this, true);
		}
	}
	Vec2 movePos = GetPos();
...
void CFatOrc::OnCollision(CCollider * _other)
{
	// 솔저와 부뒷쳤을 때
		...
		|| _other->GetObj()->GetObjType() == OBJ_TYPE::ALLY_HERO)
	{
		...
		if (m_fAttTime > 3.f)
		{
			if (GetDirAtt() == UNIT_DIRECTION::RIGHT)
				GetAnim()->PlayAnim(L"fatorc_attack_right", this, false);
			if (GetDirAtt() == UNIT_DIRECTION::LEFT)
				GetAnim()->PlayAnim(L"fatorc_attack_left", this, false);
			if (_other->GetObj()->GetObjType() == OBJ_TYPE::ALLY_UNIT)
		...
		{
			if (GetDirAtt() == UNIT_DIRECTION::RIGHT)
				GetAnim()->PlayAnim(L"fatorc_idle_right", this, true);
			if (GetDirAtt() == UNIT_DIRECTION::LEFT)
				GetAnim()->PlayAnim(L"fatorc_idle_left", this, true);
		}
	}
}